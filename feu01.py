...
def infix_reverse_polish_notation(tokens: list[str]) -> list[str]:
    ...
    for token in tokens:
        if token.isdigit():
            ...
            while (operator_stack and operator_stack[-1] != '(' and get_algo_shunting_yard(operator_stack[-1]) >= get_algo_shunting_yard(token)):
...
def evaluate_reverse_polish_notation(polish_notation: list[str]) -> list[str]:
    stack = []  # La pile pour stocker les nombres
    
    for token in polish_notation:
        if token.isdigit():  # Si c'est un nombre
            stack.append(float(token))  # Convertir en float pour gérer les divisions
        elif token in '+-*/%':  # Si c'est un opérateur
            b = stack.pop()  # Le deuxième opérande (dépilé en premier)
            a = stack.pop()  # Le premier opérande (dépilé ensuite)
            ...
            stack.append(result)  # Empiler le résultat
    
    return stack  # Retourner la pile final
...
def calculate_the_operation() -> None:
    ...
    stack = evaluate_reverse_polish_notation(polish_notation)
